...
while($row = mysqli_fetch_assoc($result)) {
 ...
 if ($pg == 6)

    {
        $pstID = $_GET['PstID'];
        $id = $_GET['sql'];
       $sql = "UPDATE users SET xApproval='$id' WHERE id='$pstID'";
        $result = mysqli_query($conn, $sql); 

        echo " <script language='JavaScript'>
        location.href='userlisting.php?sql=$pstID';
        </script>";
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                if($row["xApproval"] == "0")
                                                    {
                                                ?>
                                                    <a style="color:#000" href= "userlisting.php?PstID=<?php echo $row["id"]; ?>&&pg=6&&sql=1">Approve  </a>
                                                <?php
                                                    }
                                                else
                                                    {
                                            ?>
                                                <a style="color:#000" href= "userlisting.php?PstID=<?php echo $row["id"]; ?>&&pg=6&&sql=0">Dis-Approve</a>
                                                <?php
                                                    }
                                                ?>
